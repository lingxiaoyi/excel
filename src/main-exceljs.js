...
const xlsx = require('node-xlsx'); //用这个库格式化数据很快好用
const ExcelJS = require('exceljs');
const _ = require('lodash'); //引入lodash辅助工具
let dateUtil = require('./date-picker');
let { outputSheetDataForExceljs } = require('./util.js');
const workbook = new ExcelJS.Workbook();
const filenames = ['赵六.xlsx', '李四.xlsx', '张三.xlsx', '王五.xlsx'];
const xlsxData = filenames.map((item) => {
  return xlsx.parse(fs.readFileSync(`${__dirname}/${item}`))[0].data;
});
...
workbook.eachSheet(function (worksheet, sheetId) {
  ...
  worksheet.addImage(
    workbook.addImage({
      filename: path.resolve(__dirname, './图片1.png'),
      extension: 'png',
    }),
    {
      tl: { col: 0, row: 0 },
